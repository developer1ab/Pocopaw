...
function captureUiDump({ device, outputFile, failures }) {
  ...
  const pullResult = runAdb(device, ['pull', remotePath, outputFile]);
  if (!pullResult.success) {
    failures.push(`ui_dump_pull: ${stringValue(pullResult.stderr) || pullResult.error || 'adb pull failed'}`);
    return false;
  }

  runAdb(device, ['shell', 'rm', '-f', remotePath]);
  return true;
}
